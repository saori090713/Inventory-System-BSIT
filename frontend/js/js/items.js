...
class ItemsManager {
            ...
            if (window.isAdmin && window.isAdmin()) {
                actionsHtml = `
                    <td>
                        ...
                        <button class="btn btn-sm btn-secondary adjust-stock-btn" data-id="${item.id}">
                            Stock
                        </button>
    ...
    adjustStock(id) {
        UIHelper.showAlert('Stock adjustment feature coming soon!', 'info');
    }
    ...
    async handleFormSubmit(e) {
        ...
        try {
            ...
            if (this.currentEditId) {
                ...
                response = await apiService.updateInventoryItem(this.currentEditId, data);
                console.log('[Items] Update response:', response);
                UIHelper.showAlert(`Item "${itemName}" updated successfully`, 'success');
            } else {
                // Create new item
                response = await apiService.createInventoryItem(data);
