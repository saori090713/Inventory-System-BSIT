...
exports.update = async (req, res) => {
    try {
        ...
        if (sku && sku !== item.sku) {
            ...
            if (existingSku) {
                if (existingSku.isActive) {
                    return res.status(400).json({ error: 'An item with this SKU already exists and is active' });
                } else {
                    // Conflict with an inactive SKU - permanently delete the inactive one
                    console.log(`[INVENTORY] Deleting stale inactive item with SKU "${sku}" to resolve update conflict`);
                    await existingSku.destroy();
                }
            }
        }

        item.name = name || item.name;
        item.categoryId = category ? parseInt(category) : item.categoryId;
        ...
        item.unitId = unit ? parseInt(unit) : item.unitId;
        item.lowStockThreshold = lowStockThreshold !== undefined ? parseInt(lowStockThreshold) : item.lowStockThreshold;
