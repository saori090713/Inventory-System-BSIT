...
class UnitsManager {
    ...
    async loadUnits(search = '') {
        try {
            ...
            console.log(`✓ Loaded ${filtered.length} units`);
        } catch (error) {
    ...
    renderUnits(units) {
        ...
        if (units.length === 0) {
            tbody.innerHTML = '<tr><td colspan="3" class="text-center text-muted py-4">No units found</td></tr>';
            return;
        }
        ...
        units.forEach(unit => {
            ...
            if (isAdmin && isAdmin()) {
                actionsHtml = `
                    <td>
                        <button class="btn btn-sm btn-primary" onclick="unitsManager.editUnit(${unit.id})">
                            Edit
                        </button>
                        <button class="btn btn-sm btn-danger" onclick="unitsManager.deleteUnit(${unit.id})">
                            Delete
                        </button>
                    </td>
                `;
    ...
    async deleteUnit(id) {
        try {
            ...
            await apiService.request(`/units/${id}`, { method: 'DELETE' });
    ...
    async handleFormSubmit(e) {
        ...
        try {
            ...
            if (this.currentEditId) {
                ...
                response = await apiService.request(`/units/${this.currentEditId}`, {
                    method: 'PUT',
                ...
                // Create new unit
                response = await apiService.request('/units', {
                    method: 'POST',
                    body: JSON.stringify(data)
                });
            ...
            // Notify dashboard of data change
            window.dispatchEvent(new CustomEvent('dataChanged', { detail: { type: 'unit', action: this.currentEditId ? 'update' : 'create' } }));
